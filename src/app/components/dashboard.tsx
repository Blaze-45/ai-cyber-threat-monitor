...
import { Activity, Shield, Brain, CheckCircle2, Zap, Globe, Target, AlertTriangle, Server } from 'lucide-react';
import { CartesianGrid, Tooltip, ResponsiveContainer, Area, AreaChart, XAxis, YAxis } from 'recharts';
import { useSocket } from '../context/SocketContext';

// Helper: Generate initial history with POSITIVE values only
const generateInitialHistory = (baseTraffic: number) => {
    const history = [];
    const now = new Date();
    for (let i = 20; i >= 0; i--) { 
        ...
        // Ensure traffic is always at least 50
        let traffic = baseTraffic + (Math.random() * 100 - 50);
...
export function Dashboard() {
  ...
  useEffect(() => {
      setHistories(prev => {
          ...
          websiteStats.forEach(site => {
              if (!next[site.id]) {
                  // Ensure initial base traffic is positive
                  const safeBase = Math.max(100, site.requests || 100);
                  next[site.id] = generateInitialHistory(safeBase);
                  hasChanges = true;
              }
          });
  ...
  useEffect(() => {
    const interval = setInterval(() => {
        ...
        const timeLabel = `${currentHours}:${currentMinutes}`;
        ...
        setHistories(prev => {
            ...
            Object.keys(next).forEach(siteId => {
                ...
                // Safe base requests (never 0)
                const baseRequests = Math.max(50, siteData?.requests || 500);
                
                const fluctuation = Math.floor(Math.random() * 80 - 40); 
                
                // FORCE POSITIVE VALUES with Math.max
                const anomalyVal = isCritical 
                    ? Math.max(0, (baseRequests * 0.8) + Math.abs(fluctuation * 5)) 
                    : 0;
                
                const normalVal = isCritical 
                    ? Math.max(50, baseRequests * 0.2) 
                    : Math.max(50, baseRequests + fluctuation);

                // Time Shift Logic
                if (lastPoint && lastPoint.time !== timeLabel) {
                    currentHistory.push({
                        time: timeLabel,
                        normal: Math.floor(normalVal),
                        anomaly: Math.floor(anomalyVal)
                    });
        ...
        const isAnyCritical = statsRef.current.some(s => s.status === 'critical');
        if (isAnyCritical) {
            setBlockedCount(prev => prev + Math.floor(Math.random() * 12) + 5);
        }
